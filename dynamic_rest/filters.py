...
class DynamicSortingFilter(OrderingFilter):
    ...
    def ordering_for(self, term, view):
        ...
        if len(serializer_chain) == 1:
            field = serializer.fields.get(term)

            if field and field.source != '*':
                return field.source or term
            else:
                return None
        else:
            model_chain = []
            ...
            for segment in serializer_chain[:-1]:
                field = serializer.fields.get(segment)
            ...
            last_field = serializer.fields.get(last_segment)
    ...
    def _get_serializer_class(self, view):
        ...
        if serializer_class is None:
            msg = ("Cannot use %s on a view which does not have"
